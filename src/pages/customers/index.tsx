...
function CustomerListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CustomerInterface[]>(
    ...
    () =>
      getCustomers({
        relations: ['organization', 'sales_opportunity.count', 'service_interaction.count'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('sales_opportunity', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.sales_opportunity}</Td>
                    )}
                    {hasAccess('service_interaction', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.service_interaction}</Td>
                    )}
                    <Td>
